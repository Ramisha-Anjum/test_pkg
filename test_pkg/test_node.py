...
class MyNode(Node):
    ...
    def motion(self):
        ...
        for i in range(num_robot):
            self.cmd[i].linear.x = self.vel[0,i] * 200.5             
            self.cmd[i].linear.y = self.vel[1,i] * 200.5             #cent_with_passivity to increase speed = velocity * 200.5
            self.cmd[i].angular.x = -self.roll[i]
            self.cmd[i].angular.y = -self.pitch[i]
            self.cmd[i].angular.z = -self.yaw[i]                            
            
        self.feedback.data = [force[0,0]*400, force[1,0]*400, 0.0]
